...
export class CakedetailComponent implements OnInit {
  ...
  cake:any = {}
  isAdding:any = false
  constructor(private spinner:NgxUiLoaderService, private route : ActivatedRoute , private jagz:JagzService , private router:Router) {
    this.cakeid = this.route.snapshot.params["cakeid"]
    //api hit
    this.spinner.start()
    var url = "https://apifromashu.herokuapp.com/api/cake/" + this.cakeid
    this.jagz.getCakedetails(url).subscribe({
      next:(response:any)=>{
        this.spinner.stop()
   ...
   addToCart(){
     if(localStorage["token"]){
      ...
      this.jagz.addCakeToCart(url,body,options).subscribe({
        next:(response:any)=>{
