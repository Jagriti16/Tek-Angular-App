...
export class CartComponent implements OnInit {
  ...
  totalPrice:any = 0
  faTrash=faTrash
  constructor(private router : Router ,private jagz : JagzService , private loader: NgxUiLoaderService) {
   ...
   removeCakeFromCart(index:any){
      ...
      this.loader.start()
      this.jagz.removeCakeFromCart(url,body,options).subscribe({
        next:(response:any)=>{
          this.loader.stop()
   ...
   increaseQuantity(index:any){
     ...
     this.loader.start()
     this.jagz.addCakeToCart(url,this.cartitems[index],options).subscribe({
       next:(response:any)=>{
         this.loader.stop()
         if(response.data){
   ...
   checkout(){
     ...
     this.jagz.price = this.totalPrice
