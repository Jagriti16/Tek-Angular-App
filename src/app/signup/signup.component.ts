...
export class SignupComponent implements OnInit {
      ...
      this.http.post(url,this.userdetails).subscribe({
        next:(response:any)=>{
          ...
          if(response["message"] == "User Already Exists"){
            this.responseError = "inavalid user or user already exist";
          }
        },
        error:(error:any)=>{
  ...
  constructor(private toastr: ToastrService, private http:HttpClient, private jagz:JagzService, private formbuilder:FormBuilder) {
